...
def rk3(x, y, y_, h=0.05):
    ...
    k1 = h * F(x + h / 2, y + k0 / 2, y_ + k0_ / 2)
    k1_ = h * dif_eq(x + h / 2, y + k0 / 2, y_ + k0_ / 2)
    k2 = h * F(x + h, y - k0 + 2 * k1, y_ - k0_ + 2 * k1_)
    k2_ = h * dif_eq(x + h, y - k0 + 2 * k1, y_ - k0_ + 2 * k1_)
    y = y + (k0 + 4*k1 + k2)/6
    y_ = y_ + (k0_ + 4*k1_ + k2_)/6
...
def rk2_1(x, y, y_, h=0.05):
    k0 = h * F(x, y, y_)
    k0_ = h * dif_eq(x, y, y_)
    k1 = h * F(x + h, y + k0, y_ + k0_ / 2)
    k1_ = h * dif_eq(x + h, y + k0, y_ + k0_ / 2)
    y = y + (k0 + k1) / 2
    dy = y_ + (k0_ + k1_) / 2
    return y, dy
